...
async function sendMsg(question){if(isStreaming||!question.trim())return;isStreaming=true;send.disabled=true;input.disabled=true;addMsg(question,"user");input.value="";showTyping();try{var resp=await fetch(API_BASE+"/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:question})});if(!resp.ok)throw new Error("HTTP "+resp.status);var reader=resp.body.getReader();var decoder=new TextDecoder();var botContent="";while(true){var rd=await reader.read();if(rd.done)break;var text=decoder.decode(rd.value);var lines=text.split("\n");for(var i=0;i<lines.length;i++){var line=lines[i];if(!line.startsWith("data: "))continue;var data=JSON.parse(line.slice(6));if(data.error){botContent="Error: "+data.error;break}if(data.token){botContent+=data.token;removeTyping();updateBot(botContent)}if(data.done)break}}removeTyping();if(!botContent)updateBot("No se recibio respuesta.")}catch(e){removeTyping();updateBot("Error de conexion: "+e.message)}isStreaming=false;send.disabled=false;input.disabled=false;input.focus()}
